...
- 是可复用任务类别，不是一次性会话记录。
- 没把多个无关职责或多个独立执行合同塞进一个Skill。
- memory、模板或普通工具已足够时，不额外造Skill。
- 每次审计强制生成`split_assessment`，同时分析真实使用日志与逻辑执行合同，不凭文件长度直接决定拆分。
- 使用证据按意图、流程、输入、风险、输出/验收和职责共现关系归类，并记录来源、时间窗、命中数与盲区。
- 逻辑上分别推演候选职责A/B的单独请求、组合调用、模糊路由和高风险失败；触发、工作流、安全边界、完成标准或生命周期形成实质分叉时，才支持拆分。
- 只有内容过长而执行合同一致时，用references渐进披露；共享领域入口但子能力独立时采用router + children；无必要共同入口时才拆平级Skill。
