...
| 📚 **maishi-docs-mcp** | 通过 MCP 安全连接和使用 MAISHI Docs，按员工当前权限读写资料 |
...
*“把 Agent 接到知识库，但它只能做你本人有权做的事。”*

面向已获公司批准的 Agent 和员工：通过 OAuth 2.1 + PKCE 连接
MAISHI Docs，在不泄露凭据、不绕过权限的前提下搜索、读取、创建和更新文档。

**核心能力：**
- 一次浏览器确认后自动续期；每次调用实时沿用员工当前 Docs 权限
- 先读后改，更新携带当前版本；发生冲突不静默覆盖
- 写操作使用幂等请求键，网络异常不会重复创建内容
- 明确处理“公司未登记 Agent”“无权限”“已断开”“字段权威系统不在 Docs”等情况
- 不涉及服务器、数据库、部署、权限管理或其他人的连接

**前置条件：**
- 公司管理员已经登记并启用该 Agent 的真实 OAuth 客户端资料
- 员工本人拥有有效的 MAISHI Docs 账户
...
→ [SKILL.md](maishi-docs-mcp/SKILL.md)
...
- 当前公开版本：`skill-doctor v3.9.2` · `maishi-docs-mcp v1.0.0`
